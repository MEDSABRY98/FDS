...
export default function AlAhlyPenaltiesManagers({
    rows,
    managerType,
    onManagerTypeChange,
    managerTypeLabels,
    ...
    const controls = (
        <div className="penalties-controls">
            <DropDownList_db
                options={Object.keys(managerTypeLabels).map((key) => ({
                    value: key,
                    label: managerTypeLabels[key],
                }))}
                value={managerType}
    ...
    if (!rows || rows.length === 0) {
        return (
            <>
                {controls}
                <NoData_db message="NO PENALTY DATA IN CURRENT FILTER" />
            </>
        );
    }
                    ...
                    <tbody>
                        {paginated.map((row, idx) => (
                            <tr key={row.name}>
                                ...
                                <td className="manager-cell">{row.name}</td>
                                {cell(row.attFor)}
