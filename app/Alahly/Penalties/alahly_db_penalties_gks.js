...
export default function AlAhlyPenaltiesGKs({ rows }) {
    ...
    const emptyMessage = !rows?.length
        ? "NO PENALTY DATA"
        : searchTerm.trim()
            ? "NO GOALKEEPERS MATCH YOUR SEARCH"
            : "NO GOALKEEPERS";
            ...
            <div className="player-table-container">
                <table className="modern-player-table fade-in penalties-players-table">
                    ...
                    <thead>
                        <tr>
                            <th>#</th>
                            <SortableTh label="GOALKEEPER" sortKey="name" sortConfig={sortConfig} onSort={handleSort} />
                            <SortableTh label="TOTAL FACED" sortKey="total" sortConfig={sortConfig} onSort={handleSort} />
                            <SortableTh label="SAVED" sortKey="saved" sortConfig={sortConfig} onSort={handleSort} />
                            <SortableTh label="MISSED" sortKey="missed" sortConfig={sortConfig} onSort={handleSort} />
                            <SortableTh label="CONCEDED" sortKey="goal" sortConfig={sortConfig} onSort={handleSort} />
                            <SortableTh label="SAVE%" sortKey="savePct" sortConfig={sortConfig} onSort={handleSort} />
                        </tr>
                    </thead>
                    <tbody>
                        {paginated.length === 0 ? (
                            ...
                            paginated.map((r, i) => (
                                <tr key={r.name}>
                                    ...
                                    <td className="p-val">{r.missed}</td>
                                    <td className="g-val">{r.goal}</td>
                                    <td style={{ color: "var(--gold)" }}>{r.savePct}%</td>
                                </tr>
                            ))
                        )}
                    </tbody>
