...
import { exportIntNtContinentComparisonToExcel, EXPORT_EVENT } from "../ExcelExport/int_nt_excel_export";
...
function buildContinentComparison(matches, continent) {
    const stats = {};
    (matches || []).forEach((m) => {
        const contA = normalizeContinent(m["TEAMA CONTINENT"]);
        const contB = normalizeContinent(m["TEAMB CONTINENT"]);
        const outcome = m.OUTCOME;
        const gf = Number(m.TEAMASCORE) || 0;
        const ga = Number(m.TEAMBSCORE) || 0;
        if (contA === continent && contB) accumulateSide(stats, contB, outcome, gf, ga);
...
export default function IntNtContinents({ matches }) {
    ...
    const comparison = useMemo(() => (selectedContinent ? buildContinentComparison(matches, selectedContinent) : []), [matches, selectedContinent]);

    const continentTotals = useMemo(() => {
        if (!selectedContinent) return null;
        return comparison.reduce(
            (acc, row) => {
                acc.played += row.played; acc.wins += row.wins; acc.draws += row.draws;
                acc.losses += row.losses; acc.gf += row.gf; acc.ga += row.ga;
                return acc;
            },
    ...
    useEffect(() => {
        const handler = async (e) => {
            ...
            e.detail.claim();
            const ok = await exportIntNtContinentComparisonToExcel(e.detail.matches, selectedContinent);
            e.detail.done({ ok });
        };
    ...
    if (selectedContinent) {
        return (
            <div className="int-nt-continents">
                <button type="button" className="int-nt-continents-back" onClick={() => setSelectedContinent(null)}>← Back to Continents</button>
                <h1>{selectedContinent}</h1>
                {continentTotals && (
                    <div className="int-nt-continents-totals">
                        <span>{continentTotals.played} P</span><span>{continentTotals.wins} W</span>
                        <span>{continentTotals.draws} D</span><span>{continentTotals.losses} L</span>
                        <span>{continentTotals.gf} GF</span><span>{continentTotals.ga} GA</span>
                    </div>
                )}
                {comparison.length === 0 ? (
                    ...
                    <div className="int-nt-table-wrap">
                        <table className="int-nt-table">
                            <thead>
                                <tr><th>VS CONTINENT</th><th>P</th><th>W</th><th>D</th><th>L</th><th>GF</th><th>GA</th><th>GD</th><th>WIN %</th></tr>
                            </thead>
                            <tbody>
                                {comparison.map((row) => (
                                    <tr key={row.opponent}>
    ...
    return (
        <div className="int-nt-continents">
            <h1>CONTINENTS</h1>
            <div className="int-nt-continent-grid">
                {continents.map((c) => (
                    <button key={c.name} type="button" className="int-nt-continent-card" onClick={() => setSelectedContinent(c.name)}>
