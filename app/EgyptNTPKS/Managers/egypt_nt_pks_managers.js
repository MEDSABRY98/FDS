...
import "../HeadToHead/egypt_nt_pks_h2h.css";

export default function EgyptNTPKSManagers({ pksData }) {
    ...
    const managerStats = useMemo(() => {
        ...
        (pksData || []).forEach(kick => {
            ...
            const addMatchStat = (managerName, isEgypt) => {
                ...
                if (isEgypt) {
                    if (kick["Egypt PLAYER"]) {
                        mgrObj.shotsFor++;
                        const s = String(kick["Egypt STATUS"] || "").toUpperCase();
                        if (s.includes("GOAL") || s === "G") mgrObj.goalsFor++;
                        else if (s.includes("MISS") || s === "M" || s.includes("SAVED")) mgrObj.missesFor++;
                    }
                    if (kick["OPPONENT PLAYER"]) {
                        mgrObj.shotsAgainst++;
                        const s = String(kick["OPPONENT STATUS"] || "").toUpperCase();
                        if (s.includes("GOAL") || s === "G") mgrObj.goalsAgainst++;
                        else if (s.includes("MISS") || s === "M" || s.includes("SAVED")) mgrObj.missesAgainst++;
                    }
                } else {
                    if (kick["OPPONENT PLAYER"]) {
                        mgrObj.shotsFor++;
            ...
            if (scope === "ALL" || scope === "EGYPT") {
                addMatchStat(kick["EGYPT MANAGER"], true);
            }
    ...
    const scopeOptions = [
        ...
        { value: "EGYPT", label: "EGYPT MANAGERS" },
    ...
    return (
        <div className="pks-h2h-container fade-in">
            <div className="h2h-header-row">
                <h1 className="h2h-title">EGYPT NT <span className="gold-text">PKs MANAGERS</span></h1>
                
                <div className="h2h-controls" style={{ display: 'flex', gap: '15px', alignItems: 'center', flex: 1, maxWidth: '800px', justifyContent: 'flex-end', width: '100%', margin: '0 auto' }}>
