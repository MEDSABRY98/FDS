...
import "./egypt_nt_pks_champions.css";

export default function EgyptNTPKSChampions({ pksData }) {
    ...
    const champsStats = useMemo(() => {
        ...
        (pksData || []).forEach(kick => {
            ...
            if (kick["Egypt PLAYER"]) {
                ...
                if (status.includes("GOAL") || status === "G") champObj.goalsEgypt++;
                else if (status.includes("MISS") || status === "M" || status.includes("SAVED")) champObj.missesEgypt++;
            }

            if (kick["OPPONENT PLAYER"]) {
                champObj.shotsOpp++;
                const status = String(kick["OPPONENT STATUS"] || "").toUpperCase();
                if (status.includes("GOAL") || status === "G") champObj.goalsOpp++;
                else if (status.includes("MISS") || status === "M" || status.includes("SAVED")) champObj.missesOpp++;
            }
        });
    ...
    return (
        <div className="pks-champions-container fade-in">
            <div className="champs-header-row">
                <h1 className="champs-title">EGYPT NT <span className="gold-text">PKS CHAMPIONS</span></h1>
