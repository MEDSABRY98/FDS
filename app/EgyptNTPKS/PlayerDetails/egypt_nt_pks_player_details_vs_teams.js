...
export default function EgyptNTPKSPlayerDetailsVsTeams({ playerKicks }) {
    const stats = useMemo(() => {
        const map = {};
        playerKicks.forEach(k => {
            const opponent = k.side === "EGYPT" ? k["OPPONENT TEAM"] : "مصر";
            const t = opponent || "Unknown";
            
            if (!map[t]) map[t] = { name: t, total: 0, goals: 0, misses: 0 };
            map[t].total++;
            const s = String(k.status || "").toUpperCase();
            if (s.includes("GOAL") || s === "G") map[t].goals++;
            else map[t].misses++;
        });
    ...
    if (stats.length === 0) return <NoData_db message="NO TEAM DATA FOUND" />;
