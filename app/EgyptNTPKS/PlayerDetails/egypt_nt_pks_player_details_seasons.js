...
export default function EgyptNTPKSPlayerDetailsSeasons({ playerKicks }) {
    const stats = useMemo(() => {
        const map = {};
        playerKicks.forEach(k => {
            const sName = k["SEASON"] || "Unknown";
            if (!map[sName]) map[sName] = { name: sName, total: 0, goals: 0, misses: 0 };
            map[sName].total++;
            const sStatus = String(k.status || "").toUpperCase();
            if (sStatus.includes("GOAL") || sStatus === "G") map[sName].goals++;
            else map[sName].misses++;
        });
