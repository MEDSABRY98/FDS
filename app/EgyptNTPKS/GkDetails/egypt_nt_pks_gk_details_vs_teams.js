...
export default function EgyptNTPKSGkDetailsVsTeams({ gkKicks }) {
    const stats = useMemo(() => {
        const map = {};
        gkKicks.forEach(k => {
            const t = k.opponentTeam || "Unknown";
            if (!map[t]) map[t] = { name: t, faced: 0, saved: 0, missed: 0, conceded: 0 };
            map[t].faced++;
            const s = String(k.status || "").toUpperCase();
            if (s.includes("GOAL") || s === "G") map[t].conceded++;
            else if (k.isSave) map[t].saved++;
            else map[t].missed++;
        });
