...
export default function EgyptNTPKSGkDetailsVsPlayers({ gkKicks }) {
    const stats = useMemo(() => {
        const map = {};
        gkKicks.forEach(k => {
            const p = k.taker || "Unknown";
            if (!map[p]) map[p] = { name: p, faced: 0, saved: 0, missed: 0, conceded: 0 };
            map[p].faced++;
            const s = String(k.status || "").toUpperCase();
            if (s.includes("GOAL") || s === "G") map[p].conceded++;
            else if (k.isSave) map[p].saved++;
            else map[p].missed++;
        });
    ...
    if (stats.length === 0) return <NoData_db message="NO PLAYER DATA FOUND" />;
