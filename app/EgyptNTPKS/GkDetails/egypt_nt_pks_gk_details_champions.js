...
export default function EgyptNTPKSGkDetailsChampions({ gkKicks }) {
    const stats = useMemo(() => {
        const map = {};
        gkKicks.forEach(k => {
            const c = k["CHAMPION"] || "Unknown";
            if (!map[c]) map[c] = { name: c, faced: 0, saved: 0, missed: 0, conceded: 0 };
            map[c].faced++;
            const s = String(k.status || "").toUpperCase();
            if (s.includes("GOAL") || s === "G") map[c].conceded++;
            else if (k.isSave) map[c].saved++;
            else map[c].missed++;
        });
