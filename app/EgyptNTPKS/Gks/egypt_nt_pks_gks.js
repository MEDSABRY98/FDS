...
export default function EgyptNTPKSGKs({ pksData }) {
    ...
    const gkStats = useMemo(() => {
        ...
        (pksData || []).forEach(kick => {
            const addGkStat = (gkName, opponentPlayerName, opponentStatus, howMiss, isEgyptGK) => {
                if (!gkName || gkName === "---" || gkName === "؟" || !opponentPlayerName) return;
            ...
            // Egypt GK faces opponent kicks
            if (gkScope === "ALL" || gkScope === "EGYPT") {
                addGkStat(
                    kick["EGYPT GK"],
                    kick["OPPONENT PLAYER"],
                    kick["OPPONENT STATUS"],
                    kick["OPPONENT HOW MISS"],
                    true
                );
            }

            // Opponent GK faces Egypt kicks
    ...
    useEffect(() => {
        ...
        window.addEventListener('egyntpks-export-excel', handler);
        return () => window.removeEventListener('egyntpks-export-excel', handler);
    }, [sortedStats]);
