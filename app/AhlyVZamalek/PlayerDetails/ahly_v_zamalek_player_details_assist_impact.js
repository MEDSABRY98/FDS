...
export default function PlayerAssistImpactTable({ playerName, filteredMatches = [] }) {
    ...
    useEffect(() => {
        async function fetchImpact() {
            setLoading(true);
            const data = await AhlyVZamalekService.getPlayerAssistImpact(playerName);
    ...
    if (loading) return <div style={{ padding: '80px', textAlign: 'center', color: 'var(--player-gold)', fontFamily: 'Space Mono', letterSpacing: '2px' }}>ANALYZING ASSIST IMPACT...</div>;
    ...
    return (
        <div className="impact-section fade-in">
            ...
            <div className="impact-summary-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px', marginBottom: '40px' }}>
                ...
                >
                    <div className="card-label">TOTAL DECISIVE ASSISTS</div>
                    <div className="card-value">{totalImpact}</div>
                    <div className="card-sub">Assists that directly secured a Win or avoided Loss</div>
                </div>
                <div
                    ...
                    style={{ cursor: 'pointer' }}
                >
                    <div className="card-label">MATCH-WINNING ASSISTS</div>
                    <div className="card-value" style={{ color: '#27ae60' }}>{winCount}</div>
                    <div className="card-sub">Assists for the final winning goals</div>
                </div>
                <div
                    ...
                    style={{ cursor: 'pointer' }}
                >
                    <div className="card-label">EQUALIZING ASSISTS</div>
                    <div className="card-value" style={{ color: '#e67e22' }}>{drawCount}</div>
                    <div className="card-sub">Last assists provided to secure a draw point</div>
                </div>
            </div>
                        ...
                        <tbody>
                            {displayMatches.map((item, idx) => (
                                <tr key={idx}>
                                    ...
                                    <td style={{ color: 'var(--player-gold)', fontWeight: 800, fontSize: '16px' }}>{item.match.OPPONENT_TEAM || (item.match.TEAM_1 === 'AHLY' ? 'ZAMALEK' : 'AHLY')}</td>
