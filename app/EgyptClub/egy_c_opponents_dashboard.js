...
export default function EgyptClubOpponentsDashboard({ opponentProfile }) {
    ...
    const played = opponentProfile.played || 0;
    const wins = opponentProfile.wins || 0;
    const draws = opponentProfile.draws || 0;
    const losses = opponentProfile.losses || 0;
    const gf = opponentProfile.gf || 0; // Opponent goals
    const ga = opponentProfile.ga || 0; // Conceded by opponent
    const gd = opponentProfile.gd || 0;
    const winRate = opponentProfile.winRate || 0;
    const matches = opponentProfile.matches || [];

    // Calculate advanced stats from Opponent perspective
    const stats = useMemo(() => {
        // Clean sheets (Egypt scored 0)
        const cleanSheets = matches.filter(m => Number(m.GF) === 0).length;
        const cleanSheetRate = played > 0 ? Math.round((cleanSheets / played) * 100) : 0;

        // Failed to score (Opponent scored 0, i.e., GA === 0)
        const failedToScore = matches.filter(m => Number(m.GA) === 0).length;
        ...
        // Home / Away / Neutral breakdowns (Opponent perspective: Home is where Egypt played Away, Away is where Egypt played Home)
        const homeMatches = matches.filter(m => m["H-A-N"] === "A"); // Egypt Away = Opponent Home
        const homeWins = homeMatches.filter(m => m["W-D-L"] === "L").length;
        const homeDraws = homeMatches.filter(m => m["W-D-L"] && m["W-D-L"].startsWith("D")).length;
        const homeLosses = homeMatches.filter(m => m["W-D-L"] === "W").length;
        const homeGf = homeMatches.reduce((sum, m) => sum + (Number(m.GA) || 0), 0); // GA is opponent goals
        const homeGa = homeMatches.reduce((sum, m) => sum + (Number(m.GF) || 0), 0); // GF is Egypt goals
        const homeWinRate = homeMatches.length > 0 ? Math.round((homeWins / homeMatches.length) * 100) : 0;

        const awayMatches = matches.filter(m => m["H-A-N"] === "H"); // Egypt Home = Opponent Away
        const awayWins = awayMatches.filter(m => m["W-D-L"] === "L").length;
        const awayDraws = awayMatches.filter(m => m["W-D-L"] && m["W-D-L"].startsWith("D")).length;
        const awayLosses = awayMatches.filter(m => m["W-D-L"] === "W").length;
        const awayGf = awayMatches.reduce((sum, m) => sum + (Number(m.GA) || 0), 0);
        const awayGa = awayMatches.reduce((sum, m) => sum + (Number(m.GF) || 0), 0);
        ...
        const neutralWins = neutralMatches.filter(m => m["W-D-L"] === "L").length;
        const neutralDraws = neutralMatches.filter(m => m["W-D-L"] && m["W-D-L"].startsWith("D")).length;
        const neutralLosses = neutralMatches.filter(m => m["W-D-L"] === "W").length;
        const neutralGf = neutralMatches.reduce((sum, m) => sum + (Number(m.GA) || 0), 0);
        const neutralGa = neutralMatches.reduce((sum, m) => sum + (Number(m.GF) || 0), 0);
        const neutralWinRate = neutralMatches.length > 0 ? Math.round((neutralWins / neutralMatches.length) * 100) : 0;

        // Biggest Win (Opponent won, i.e., W-D-L === "L" from Egypt perspective)
        let biggestWin = null;
        let maxWinDiff = -1;
        matches.forEach(m => {
            if (m["W-D-L"] === "L") {
                const diff = (Number(m.GA) || 0) - (Number(m.GF) || 0); // Opponent goals - Egypt goals
                if (diff > maxWinDiff) {
                    maxWinDiff = diff;
                    biggestWin = m;
                } else if (diff === maxWinDiff && biggestWin) {
                    if ((Number(m.GA) || 0) > (Number(biggestWin.GA) || 0)) {
                        biggestWin = m;
                    }
                }
            }
        ...
        // Biggest Loss (Opponent lost, i.e., W-D-L === "W" from Egypt perspective)
        let biggestLoss = null;
        let maxLossDiff = -1;
        matches.forEach(m => {
            if (m["W-D-L"] === "W") {
                const diff = (Number(m.GF) || 0) - (Number(m.GA) || 0); // Egypt goals - Opponent goals
                if (diff > maxLossDiff) {
                    maxLossDiff = diff;
                    biggestLoss = m;
                } else if (diff === maxLossDiff && biggestLoss) {
                    if ((Number(m.GF) || 0) > (Number(biggestLoss.GF) || 0)) {
                        biggestLoss = m;
                    }
                }
            }
        ...
        <div className="profile-dashboard fade-in" style={{ paddingBottom: '20px' }}>
            ...
            <div className="kpi-section-grid">
                {[
                    ...
                    { label: "OPPONENT WINS", val: wins, color: '#00c853', icon: <TrophyIcon color="#00c853" /> },
                    { label: "DRAWS", val: draws, color: 'var(--gold, #c9a84c)', icon: <DrawsIcon color="var(--gold, #c9a84c)" /> },
                    { label: "OPPONENT LOSSES", val: losses, color: '#ff4d4d', icon: <LossIcon color="#ff4d4d" /> },
                    { label: "GOALS SCORED", val: gf, icon: <FlameIcon /> },
                    { label: "GOALS CONCEDED", val: ga, icon: <ShieldIcon /> },
            ...
            <div className="details-grid">
                ...
                <div className="details-section-card">
                    ...
                    <h4 className="sub-section-title"><MapPin size={14} style={{ color: '#888' }} /> VENUE BREAKDOWN (OPPONENT PERSPECTIVE)</h4>
                            ...
                            {stats.biggestWin ? (
                                <div className="record-match-body">
                                    ...
                                    <div className="record-match-score">
                                        Score: <span className="score-highlight green">{stats.biggestWin.GA} - {stats.biggestWin.GF}</span>
                        ...
                        <div className="record-match-box" style={{ borderLeft: '4px solid #ff4d4d' }}>
                            ...
                            {stats.biggestLoss ? (
                                <div className="record-match-body">
                                    ...
                                    <div className="record-match-score">
                                        Score: <span className="score-highlight red">{stats.biggestLoss.GA} - {stats.biggestLoss.GF}</span>
                        ...
                        <div className="record-match-box" style={{ borderLeft: '4px solid var(--gold, #c9a84c)' }}>
                            ...
                            {stats.highestScoring ? (
                                <div className="record-match-body">
                                    ...
                                    <div className="record-match-score">
                                        Score: <span className="score-highlight gold">{stats.highestScoring.GA} - {stats.highestScoring.GF}</span>
