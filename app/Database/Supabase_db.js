...
function isRowIdOnlyCatalogSort(sorting, columns = []) {
    if (!sorting) return true;

    const trimmed = String(sorting).trim();
    if (trimmed.toUpperCase() === "ROW_ID") return true;

    if (trimmed.startsWith("{")) {
        try {
            const parsed = JSON.parse(trimmed);
            const preset = String(parsed?.dataSortPreset || "").trim().toUpperCase();
            if (preset === "ROW_ID" || !preset) {
                const rules = getActiveTableSortRules(parseTableSortSetting(sorting, columns));
                return rules.length === 1
                    && String(rules[0]?.column || "").toUpperCase() === "ROW_ID"
                    && String(rules[0]?.direction || "").toLowerCase() === "desc";
            }
        } catch {
            return false;
        }
...
export function sortGlobalDbManagementTableData(rows, columns = [], sorting = null) {
    ...
    const namePair = findCatalogNamePairInColumns(columns);
    if (namePair && isRowIdOnlyCatalogSort(sorting, columns)) {
        const [arCol, enCol] = namePair;
        return sortCatalogRowsArThenEn(rows, arCol, enCol);
    }
