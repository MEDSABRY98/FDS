...
import AlAhlyPKsPlayerDetails from "../PlayerDetails/alahly_pks_player_details";
import NoData_db from "../../lib/NoData_db";
import SearchBar_db from "../../lib/SearchBar_db";
import DropDownList_db from "../../lib/DropDownList_db";
import "./alahly_pks_players.css";

export default function AlAhlyPKsPlayers({ pksData }) {
    const [searchTerm, setSearchTerm] = useState("");
    const [playerScope, setPlayerScope] = useState("ALL"); // ALL, AHLY, OPPONENTS
    const [currentPage, setCurrentPage] = useState(1);
    const [selectedPlayer, setSelectedPlayer] = useState(null);
    const [sortConfig, setSortConfig] = useState({ key: 'total', direction: 'desc' });
    const pageSize = 50;

    // Aggregate statistics by player and scope
    const playerStats = useMemo(() => {
        ...
        (pksData || []).forEach(kick => {
            ...
            if (playerScope === "ALL" || playerScope === "AHLY") {
                addStat(kick["AHLY PLAYER"], kick["AHLY STATUS"], "AHLY");
            }
    ...
    const scopeOptions = [
        ...
        { value: "AHLY", label: "AL AHLY PLAYERS" },
        ...
        <div className="pks-players-container fade-in">
            {!selectedPlayer && (
                <div className="players-header-section">
                    <h1 className="players-title">AL AHLY <span className="gold-text">PKs PLAYERS</span></h1>
