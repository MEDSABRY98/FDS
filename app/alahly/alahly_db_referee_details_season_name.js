...
export default function Referee_SeasonName_Module({ stats }) {
    ...
    const sortedChamps = useMemo(() => {
        return Object.keys(statsByChampSeason).sort((a, b) => b.localeCompare(a));
    }, [statsByChampSeason]);
        ...
        <div className="tab-content" style={{ paddingTop: '10px' }}>
            <div className="seasons-wrap" style={{ maxWidth: '1450px', width: '98%', margin: '0 auto' }}>
                <div className="section-title" style={{ fontSize: '24px', color: 'var(--player-gold)', fontFamily: 'Bebas Neue', letterSpacing: '2px', marginBottom: '10px', textTransform: 'uppercase' }}>
                    REFEREE PERFORMANCE <span className="accent" style={{ color: '#fff' }}>- SEASON NAME</span>
                </div>
                ...
                {sortedChamps.length === 0 ? (
                    ...
                    sortedChamps.map(champ => {
                        ...
                        const totals = { MP: 0, W: 0, D: 0, L: 0, GS: 0, GA: 0, PF: 0, PA: 0, CS_FOR: 0, CS_AGN: 0 };
                                    ...
                                    <table className="analysis-table" style={{ width: '100%', borderCollapse: 'collapse', color: '#333' }}>
                                        <thead>
                                            <tr style={{ background: '#f8f9fa' }}>
                                                ...
                                                <th style={{ padding: '16px 15px', textAlign: 'center', fontSize: '13px', fontFamily: 'Space Mono', color: '#999', borderBottom: '2px solid #eee', letterSpacing: '2px', fontWeight: '700' }}>PEN F-A</th>
                                                <th style={{ padding: '16px 15px', textAlign: 'center', fontSize: '13px', fontFamily: 'Space Mono', color: '#999', borderBottom: '2px solid #eee', letterSpacing: '2px', fontWeight: '700' }}>CS F-A</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {sortedSeasons.map((season, idx) => {
                                                ...
                                                return (
                                                    <tr key={season} style={{ background: idx % 2 === 0 ? '#fff' : '#fafafa', borderBottom: '1px solid #f0f0f0' }}>
                                                        ...
                                                        <td style={{ padding: '18px 15px', textAlign: 'center', fontSize: '16px', color: '#27ae60', fontWeight: '900', fontFamily: 'Outfit' }}>{s.wins}</td>
                                                        ...
                                                        <td style={{ padding: '18px 15px', textAlign: 'center', fontSize: '15px', color: '#000', fontWeight: '800', fontFamily: 'Outfit' }}>{s.penFor} - {s.penAgainst}</td>
                                                        <td style={{ padding: '18px 15px', textAlign: 'center', fontSize: '15px', color: '#2ecc71', fontWeight: '800', fontFamily: 'Outfit' }}>{s.csFor} / <span style={{ color: '#ff6b6b' }}>{s.csAgainst}</span></td>
                                                    </tr>
                                                );
                                            })}
                                        </tbody>
                                        <tfoot>
                                            <tr style={{ background: '#0a0a0a', color: '#fff', fontWeight: '700' }}>
                                                ...
                                                <td style={{ padding: '20px', textAlign: 'center', fontSize: '20px', color: '#fff', fontFamily: 'Outfit', fontWeight: '900' }}>{totals.PF} - {totals.PA}</td>
                                                <td style={{ padding: '20px', textAlign: 'center', color: '#2ecc71', fontSize: '20px', fontFamily: 'Outfit', fontWeight: '900' }}>{totals.CS_FOR} / <span style={{ color: '#ff6b6b' }}>{totals.CS_AGN}</span></td>
                                            </tr>
                                        </tfoot>
                                    </table>
                                </div>
