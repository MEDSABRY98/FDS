export default function EgyptNTPlayersPenalties({ paginatedRows, currentPage, pageSize, handleSort, renderSortIcon, setSelectedPlayer }) {
    return (
        <table className="modern-player-table fade-in" style={{ tableLayout: 'fixed' }}>
            ...
            <thead>
                <tr>
                    ...
                    <th onClick={() => handleSort('total')} className="sortable">SHOTS {renderSortIcon('total')}</th>
                    <th onClick={() => handleSort('goal')} className="sortable">GOALS {renderSortIcon('goal')}</th>
            ...
            <tbody>
                {paginatedRows.map((r, i) => (
                    <tr key={r.name}>
                        ...
                        <td style={{ color: 'var(--gold)' }}>{r.total}</td>
