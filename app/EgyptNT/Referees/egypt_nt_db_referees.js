...
import EgyptNTRefereeDetails from "../RefereeDetails/egypt_nt_db_referee_details";
...
export default function EgyptNTReferees({ matches, playerDetails, howPenMissed }) {
    ...
    const stats = useMemo(() => {
        ...
        (matches || []).forEach(m => {
            ...
            s.penFor += penForEgypt;
            s.penAgainst += penAgainstEgypt;
        });
        ...
        <div className="tab-content fade-in" id="tab-referees">
            <div className="referee-premium-wrap">
                <div className="header-tabs-container">
                    <div className="section-title">EGYPT NT <span className="accent">REFEREES</span></div>
                </div>
                ...
                {totalPages > 1 && (
                    <div className="pagination-referee">
                        <button className="page-btn prev-btn" onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1}>←</button>
                        <div className="page-info">PAGE <span className="p-num">{currentPage}</span> OF <span className="p-num">{totalPages}</span></div>
                        <button className="page-btn next-btn" onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages}>→</button>
                    </div>
                )}
            </div>
        </div>
