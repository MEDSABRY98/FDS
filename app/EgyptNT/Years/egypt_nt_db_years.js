...
export default function EgyptNTYears({ matches }) {
    ...
    const grandTotals = useMemo(() => {
        ...
        Object.values(statsByYear).forEach(s => {
            t.MP += s.MP; t.W += s.W; t.DP += s.DP; t.DN += s.DN; t.L += s.L;
            t.GF += s.GF; t.GA += s.GA; t.CSF += s.CSF; t.CSA += s.CSA;
        });
        return t;
    }, [statsByYear]);
    ...
    return (
        <div className="tab-content" id="tab-years">
            <div className="seasons-wrap" style={{ maxWidth: '1400px', width: '95%', margin: '0 auto', paddingTop: '0' }}>
                <div className="section-title">EGYPT NT <span className="accent">YEARS</span></div>
