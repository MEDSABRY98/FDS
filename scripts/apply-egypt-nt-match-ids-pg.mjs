/**
 * Apply Egypt NT MATCH_ID migration via direct Postgres (DATABASE_URL).
...
const ALL_TABLES = ["egy_NT_MATCHDETAILS", ...LINKED_TABLES];
...
async function main() {
    ...
    console.log("=== APPLY Egypt NT (Postgres bulk) ===");

    await client.query("SET statement_timeout = 0");
    await runSqlFile("scripts/alter-egypt-nt-match-id-temp.sql");

    const teams = await fetchAll("db_TEAMS", '"TEAM_ID", "TEAM_NAME", "TEAM_NAME_EN"');
    const plan = buildEgyptNtMatchMigrationPlan(
        await fetchAll(
            "egy_NT_MATCHDETAILS",
            '"ROW_ID", "MATCH_ID", "AGE", "Egypt TEAM", "OPPONENT TEAM", "DATE"'
        ),
