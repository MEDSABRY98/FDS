/**
 * Audit Al Ahly MATCH_ID migration (TEAM_ID + Excel date).
 * Usage: node scripts/audit-alahly-match-ids.mjs
 */
...
const matches = await fetchAllRows(
    supabase,
    "alahly_MATCHDETAILS",
    'ROW_ID, MATCH_ID, "OPPONENT TEAM", DATE, CHAMPION'
);

const plan = buildMatchMigrationPlan(matches, teamLookup);
...
console.log("=== Al Ahly MATCH_ID Audit ===");
...
if (plan.duplicateGroups.length) {
    console.log(`\nCollisions (same TEAM_ID+DATE): ${plan.duplicateGroups.length}`);
...
const outPath = "scripts/audit-alahly-match-ids-report.json";
writeFileSync(outPath, JSON.stringify(report, null, 2), "utf8");
console.log(`\nFull report: ${outPath}`);

if (plan.stats.collisions > 0 || plan.stats.unresolved > 0) {
    console.log("\n⚠ Fix collisions/unresolved before running migrate --apply");
    process.exit(1);
}
